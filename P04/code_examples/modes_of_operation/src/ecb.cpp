/*
 * Example of ECB mode of operation using AES
 */
...
// Encrypting AES / ECB mode
std::string encrypt(std::string input, CryptoPP::ECB_Mode<CryptoPP::AES>::Encryption encryptor) {
    std::string result;
...
// Decrypting AES / ECB mode
std::string decrypt(std::string input, CryptoPP::ECB_Mode<CryptoPP::AES>::Decryption decryptor) {
    std::string result;
...
int main(void) {
    ...
    // Input message
    std::string input = "Hello world!, this is a very long message to test ECB mode with AES provided by the Crypto++ library!";
    std::string cipher, encoded, recovered;

    // ECB Encryptor and Decryptor
    CryptoPP::ECB_Mode<CryptoPP::AES>::Encryption encryptor(key, key.size());
    CryptoPP::ECB_Mode<CryptoPP::AES>::Decryption decryptor(key, key.size());
