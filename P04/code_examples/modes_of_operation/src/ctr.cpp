/*
 * Example of CTR mode of operation using AES
 */
...
// Encrypting AES / CTR mode
std::string encrypt(std::string input, CryptoPP::CTR_Mode<CryptoPP::AES>::Encryption encryptor) {
    std::string result;
...
// Decrypting AES / CTR mode
std::string decrypt(std::string input, CryptoPP::CTR_Mode<CryptoPP::AES>::Decryption decryptor) {
    std::string result;
...
int main(void) {
    ...
    // Input message
    std::string input = "Hello world!, this is a very long message to test CTR mode with AES provided by the Crypto++ library!";
    std::string cipher, encoded, recovered;

    // CTR Encryptor and Decryptor
