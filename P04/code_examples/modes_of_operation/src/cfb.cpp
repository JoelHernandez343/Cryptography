/*
 * Example of CFB mode of operation using AES
 */
...
// Encrypting AES / CFB mode
std::string encrypt(std::string input, CryptoPP::CFB_Mode<CryptoPP::AES>::Encryption encryptor) {
    std::string result;
...
// Decrypting AES / CFB mode
std::string decrypt(std::string input, CryptoPP::CFB_Mode<CryptoPP::AES>::Decryption decryptor) {
    std::string result;
...
int main(void) {
    ...
    // Input message
    std::string input = "Hello world!, this is a very long message to test CFB mode with AES provided by the Crypto++ library!";
    std::string cipher, encoded, recovered;

    // CFB Encryptor and Decryptor
