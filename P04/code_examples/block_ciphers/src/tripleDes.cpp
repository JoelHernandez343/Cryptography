/*
 * Example of TripleDES usage with CBC mode of operation
 */
...
// Encrypting TripleDES / CBC mode
std::string encrypt(std::string input, CryptoPP::CBC_Mode<CryptoPP::DES_EDE3>::Encryption encryptor) {
    std::string result;
...
// Decrypting TripleDES / CBC mode
std::string decrypt(std::string input, CryptoPP::CBC_Mode<CryptoPP::DES_EDE3>::Decryption decryptor) {
    std::string result;
...
int main(void) {
    ...
    // Input message
    std::string input = "Hello world!, this is a very long message to prove the TripleDES encryption with CBC mode provided by the Crypto++ library!";
    std::string cipher, encoded, recovered;

    // CBC Encryptor and Decryptor
    CryptoPP::CBC_Mode<CryptoPP::DES_EDE3>::Encryption encryptor(key, key.size(), iv);
    CryptoPP::CBC_Mode<CryptoPP::DES_EDE3>::Decryption decryptor(key, key.size(), iv);
